...
baseline_readout: Module = Identity()
...
policy_readout: Module = Softmax(dim=2)
...
baseline_modules: List[Module] = [baseline_net]
policy_modules: List[Module] = [policy_net]
recurrent_modules: List[SGRUHCell] = [baseline_net, policy_net]
all_modules: List[Module] = baseline_modules + policy_modules
...
MOVES_HARD_LIMIT: int = ALLEY_LEN * 10  # min(10 * ALLEY_LEN, 1000)
DISCOUNT: float = 0.98
    ...
    for stint_nr in range(STINTS_PER_EPOCH):
        ...
        for game_nr in range(GAMES_PER_STINT):
            ...
            while move_idx < MOVES_HARD_LIMIT - 1 and not train_game.gameover:
                ...
                # Get current state
                curr_state: Tensor = (
                    th.tensor(train_game.percepts).float().reshape(1, 1, -1).to(device)
                )
                ...
                est_return = 4.1 * sigmoid(est_return.sum()) - 0.1
                est_return = est_return.reshape(1)
