...
for alley_len_iter in range(MIN_ALLEY_LEN, MAX_ALLEY_LEN + 1):
    ...
    for replica_nr in range(REPLICAS):
        ...
        # ---- OPTIMIZERS ----
        BASELINE_OPTIM = SGD(baseline_cell.parameters(), lr=1e-2, momentum=0.9)
        POLICY_OPTIM = SGD(policy_cell.parameters(), lr=1e-2, momentum=0.9)
    ...
    ax.grid()
    plt.savefig("plots/tmaze_{}_original_rew.png".format(alley_len_iter))
    ...
    ax.grid()
    plt.savefig("plots/tmaze_{}_original_mov.png".format(alley_len_iter))
